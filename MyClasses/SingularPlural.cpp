...
void SingularPlural::addNewToList(string d1,string d2)
{
    WordsMeanings::addNewToList(d1,d2);
}

void SingularPlural::loadDataFromFile()
{
  string read1 = "";
  string read2 = "";

  ifstream load;
  load.open("SingularPlural.txt");
  if(!load)
  {
...
void SingularPlural::saveInFile()
{
    Node* triverse = start;
    ofstream save;
    save.open("SingularPlural.txt");
...
bool SingularPlural::alreadyExist(string d1)
{
    return WordsMeanings::alreadyExist(d1);
}

string SingularPlural::findData(string data)
{
   return WordsMeanings::findData(data);
}

void SingularPlural::releaseMemory()
{
